    ...
    public static void main(String[] args) {
        for (int i = 0; i < args.length; i++) {
            ...
            if (arg.toLowerCase().endsWith(".mzxml")) {
                 String filein = arg;
                 String fileout = arg.toLowerCase().replace(".mzxml", "short.mzxml");
                 XTandemSaxUtilities.limitTagCount(filein, fileout, "mzXML", "scan", MAX_SCANS);
                continue;
             }
            if (arg.toLowerCase().endsWith(".mzml")) {
                 String filein = arg;
                 String fileout = arg.toLowerCase().replace(".mzml", "short.mzml");
                 XTandemSaxUtilities.limitTagCount(filein, fileout, "indexedmzML", "spectrum", MAX_SCANS);
                continue;
              }
         }
