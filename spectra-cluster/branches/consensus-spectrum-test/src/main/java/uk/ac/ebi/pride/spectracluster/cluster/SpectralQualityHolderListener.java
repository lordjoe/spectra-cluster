...
/**
 * uk.ac.ebi.uk.ac.ebi.pride.spectracluster.cluster.SpectralQualityHolder
 ...
 * Date: 7/10/13
 */
public class SpectralQualityHolderListener implements ISpectrumHolderListener {
    ...
    public void update(SpectrumHolderEvent event) {
        switch (event.getType()) {
            case ADD:
                onSpectraAdd(event.getSpectra());
                break;
            case REMOVE:
                onSpectraRemove(event.getSpectra());
                break;
        }
    }

    public void onSpectraAdd(List<ISpectrum> added) {
        for (ISpectrum add : added) {
            handleQualityInsert(add);
        }
    }

    public void onSpectraRemove(List<ISpectrum> removed) {
        for (ISpectrum remove : removed) {
            handleQualityRemove(remove);
        }
     }
}
