...
public class FrankEtAClusterEngineTest {
    ...
    public void testBuildConsensusSpectrum() throws Exception {
        ...
        for (ConsensusSpectraItems cluster : consensusSpectraItems) {
            ISpectrum consensusSpectrum = cluster.getConsensus();
            List<ISpectrum> spectra = cluster.getSpectra();
            IConsensusSpectrumBuilder consensusSpectrumBuilder =  new ConsensusSpectrum();
            SpectrumHolderEvent event = new SpectrumHolderEvent(consensusSpectrumBuilder, SpectrumHolderEvent.Type.ADD, spectra.toArray(new ISpectrum[spectra.size()]));
            consensusSpectrumBuilder.update(event);
