...
public class BinnedHighestNPeakFilter implements IPeakFilter {
    ...
    public static final int MINIMUM_BINNED_MZ = 300;     // todo replace with an agreed global
    public static final int MAXIMUM_BINNED_MZ = 1500;     // todo replace with an agreed global
    ...
    public BinnedHighestNPeakFilter( int maxPeaks) {
         this(maxPeaks,DEFAULT_BIN_SIZE,DEFAULT_BIN_OVERLAP);
     }
    ...
    @Override public List<IPeak> filter(List<IPeak> peaks) {
        ...
        int startpeak = 0;
        for (int binBottom = MINIMUM_BINNED_MZ; binBottom < MAXIMUM_BINNED_MZ - binSize; binBottom += binOverlap) {
    ...
    protected int handleBin(List<IPeak> allpeaks, int startpeak, Set<IPeak> retained, int binBottom) {
        List<IPeak>  byIntensity = new ArrayList<IPeak>() ;
        int nextBin = 0;
        if(true)
             throw new UnsupportedOperationException("Populate Peaks in bin"); // ToDo
        ...
        if(true)
             throw new UnsupportedOperationException("Find first peak in next bin"); // ToDo
        return nextBin;
