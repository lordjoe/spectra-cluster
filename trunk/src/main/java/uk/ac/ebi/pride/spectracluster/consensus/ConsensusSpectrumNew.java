...
public class ConsensusSpectrumNew implements IConsensusSpectrumBuilder {
    ...
    protected void update() {
        ...
        if (consensusPeaks.size() < 1) {
            setIsDirty(false);
            List<IPeak> peaksList = new ArrayList<IPeak>();
            consensusSpectrum = new PeptideSpectrumMatch(id, null, averageCharge, averagePrecursorMz, peaksList);
            return;
        }
     ...
     */
    protected void adaptPeakIntensities() {
     ...
     * MZ_THRESHOLD_STEP.
     */
    protected void mergeIdenticalPeaks() {
        List<IPeak> consensusPeaksX = new ArrayList<IPeak>(consensusPeaks);
        List<IPeak> newPeaks = mergeIdenticalPeaksInternal(consensusPeaksX);
        consensusPeaks.clear();
        consensusPeaks.addAll(newPeaks);
    }

    /**
     * extracted for compatability with old implementation and tests
     * @param pConsensusPeaksX
     */
    protected  List<IPeak> mergeIdenticalPeaksInternal(final List<IPeak> pConsensusPeaksX) {
        List<IPeak> newPeakList = new ArrayList<IPeak>(pConsensusPeaksX);
        for (float range = MZ_THRESHOLD_STEP; range <= FINAL_MZ_THRESHOLD; range += MZ_THRESHOLD_STEP) {

            List<IPeak> newerPeakList =  mergeRangedPeaks(range, newPeakList);

            newPeakList.clear();
            newPeakList.addAll(newerPeakList);
        }
        return newPeakList;
    }

    protected  List<IPeak>  mergeRangedPeaks(final float pRange, final List<IPeak> pNewPeakList) {
        List<IPeak> newPeakList = new ArrayList<IPeak>(pNewPeakList);
        for (int i = 0; i < newPeakList.size() - 1; i++) {
            IPeak currentPeak = newPeakList.get(i);
            IPeak nextPeak = newPeakList.get(i + 1);

            // check whether the next peak should be considered identical to the current one
            if (nextPeak.getMz() <= currentPeak.getMz() + pRange) {
                ...
                newPeakList.add(pNewPeakList.get(i));
            }
        }

        newPeakList.add(newPeakList.get(newPeakList.size() - 1));
        return newPeakList;
    }
    ...
    /**
      ...
      public IConsensusSpectrumBuilder cloneSpectrumBuilder() {
          return new ConsensusSpectrumNew();
      }
